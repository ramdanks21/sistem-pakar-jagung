<title>Diagnosa - Certainty Factor V.2</title>
<?php
switch ($_GET['act']) {
	...
	if ($_POST['submit']) {
		...
		$sqlpkt = mysql_query("SELECT * FROM hama order by kode_hama+0");
...
// --------------------- START ------------------------
		$sqlhama = mysql_query("SELECT * FROM hama order by kode_hama");
		$arhama = array();
		while ($rhama = mysql_fetch_array($sqlhama)) {
			$cftotal_temp = 0;
			$cf = 0;
			$sqlgejala = mysql_query("SELECT * FROM basis_pengetahuan_hama where kode_hama=$rhama[kode_hama]");
			$cflama = 0;
			...
			if ($cflama > 0) {
				$arhama += array($rhama[kode_hama] => number_format($cflama, 4));
			}
		}
...
// --------------------- END -------------------------
		...
		foreach ($argejala as $key => $value) {
			...
			$ig++;
			$gejala = $key;
			$sql4 = mysql_query("SELECT * FROM keparahan_penyakit where kode_gejala = '$key'");
		...
		if ($argpkt[$idpkt[1]]) {
			$gambar = 'gambar/hama/' . $argpkt[$idpkt[1]];
		} else {
		...
		echo "<div class='callout callout-default'>Jenis hama yang diderita adalah <b><h3 class='text text-success'>" . $nmpkt[1] . "</b> / " . round($vlpkt[1], 2) . " % (" . $vlpkt[1] . ")<br></h3>";
		...
		echo "
		<h2 class='text text-primary'>Diagnosa hama</h2>  <hr>
		...
		<form name=text_form method=POST action='diagnosahama' >
		<table class='table table-bordered table-striped konsultasi'><tbody class='pilihkondisi'>

		<tr>
		<th>No</th>
		<th>L.Daun Terserang</th>
		<th>SKor</th>
		<th>Jumlah Daun Sama</th>
		<th>Aksi</th>
		</tr>";
